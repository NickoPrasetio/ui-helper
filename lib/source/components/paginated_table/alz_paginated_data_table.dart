/*
  alz_paginated_data_table.dart
  
  Created by GABRIELSORE on 11/05/24.
...
class AlzPaginatedDataTable extends StatefulWidget {
  ...
  // To Show and Hide Pagination Number Footer
  final bool isFooterEnable;

  @override
  AlzPaginatedDataTableState createState() => AlzPaginatedDataTableState();
}

/// Holds the state of a [PaginatedDataTable].
///
/// The table can be programmatically paged using the [pageTo] method.
class AlzPaginatedDataTableState extends State<AlzPaginatedDataTable> {
  ...
  @override
  void didUpdateWidget(AlzPaginatedDataTable oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    double currentPage = (_firstRowIndex / widget.rowsPerPage) + 1;
    String footerPaginationText = widget.footerText.replaceAll('\$1', '${currentPage.toInt()}');
    footerPaginationText =
        footerPaginationText.replaceAll('\$2', '${(_rowCount / widget.rowsPerPage).ceil()}');
    ...
    if (widget.onRowsPerPageChanged != null) {
      ...
      footerWidgets.addAll(<Widget>[
        ...
        AdaptiveText(
            ...
            fontSize: UIFont.h6,
            color: ColorPalette.textCarchoalGreyColor),
        ConstrainedBox(
                ...
                child: DropdownButtonHideUnderline(
                  child: ButtonTheme(
                      ...
                      child: DropdownButton<int>(
                        ...
                        selectedItemBuilder: (BuildContext context) => widget.availableRowsPerPage
                            .map<Widget>((int item) => Row(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  children: <Widget>[
                                    AdaptiveText(text: item.toString(), fontSize: UIFont.h6),
                                  ],
                                ))
                            .toList(),
                        items: widget.availableRowsPerPage.map<DropdownMenuItem<int>>((int value) {
                          return DropdownMenuItem<int>(
                              value: value,
                              child: SizedBox(
                                  width: 100,
                                    ...
                                    children: <Widget>[
                                      value == widget.rowsPerPage
                                          ? const Icon(Icons.check,
                                              color: ColorPalette.frenchBlue, size: 15)
                                          : const SizedBox(width: 15),
                                      const SizedBox(width: 4),
                                      AdaptiveText(text: '$value'),
                                    ],
                                  )));
                        }).toList(),
                        value: widget.rowsPerPage,
    ...
    if (widget.isFooterEnable) {
      footerWidgets.addAll(<Widget>[
        ...
        Semantics(
          label: SemanticsConstant.buttonIllustHistoryPrevious,
          container: true,
          child: IconButton(
            icon: RotatedBox(
                quarterTurns: 2,
                child: Image.asset('assets/images/ArrowBlue.png',
                    color: _firstRowIndex <= 0 ? ColorPalette.gray3 : ColorPalette.frenchBlue,
                    width: 24,
                    height: 24)),
            padding: EdgeInsets.zero,
        ...
        AdaptiveText(
          text: footerPaginationText,
          color: ColorPalette.carbonGrey,
        ),
        Semantics(
          label: SemanticsConstant.buttonIllustHistoryNext,
          container: true,
          child: IconButton(
            icon: Image.asset(
              'assets/images/ArrowBlue.png',
              width: 24,
              height: 24,
              color: _isNextPageUnavailable() ? ColorPalette.gray3 : ColorPalette.frenchBlue,
            ),
            padding: EdgeInsets.zero,
